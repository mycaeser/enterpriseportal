 ...
 */
$(function(){
	var getBriefUrl='/enterpriseportal/frontend/getbriefintroduction';
	var updateBriefUrl='/enterpriseportal/cps/modifybrief';
	var getAboutUsUrl='/enterpriseportal/companygx/getaboutus';
	var getCultureUrl='/enterpriseportal/companygx/getculture';
	var updateCultureUrl='/enterpriseportal/cps/modifyindexculture';
	var getTeamUrl='/enterpriseportal/companygx/getteamlist';
	var deleteTeamByIdUrl='/enterpriseportal/cps/deleteteambyid';
	var getTeamByIdUrl='/enterpriseportal/cps/getteambyid?id=';
	var updateTeamUrl='/enterpriseportal/cps/updateteambyid';
	var getHonorUrl='/enterpriseportal/companygx/gethonorlist';
	var deleteHonorUrl='/enterpriseportal/cps/deletehonorbyid';
	var getHonoryByIdUrl='/enterpriseportal/cps/gethonorbyid?id=';
	var updateHonorUrl='/enterpriseportal/cps/updatehonorbyid';
	var imgUrlfix='/';
	var inHtml='';
			...
			if(inputStr.length>500){
				...
				$.ajax({
					...
					success : function(data) {
						alert('提交成功');
						self.location='/enterpriseportal/cps/brief?a=1';
					}
				});
			}
		});
		...
		$('.cabtn').click(function(){
			...
			if(inputStr.length>2000){
				...
				$.ajax({
					...
					success : function(data) {
							alert('修改成功');
							self.location='/enterpriseportal/cps/brief?a=2';
					}
				});
			}
		});
		...
		$('.cabtn').click(function(){
			...
			if(inputStr1.length>2000||inputStr2.length>2000||inputStr3.length>2000){
				...
				$.ajax({
					...
					success : function(data) {
							alert('修改成功');
							self.location='/enterpriseportal/cps/brief?a=3';
					}
				});
			}
		});
	...
	if(ma!=null&&ma==4){//显示团队管理
		...
		$.getJSON(getTeamUrl,function(data){
			...
			teamObj.map(function(item,data){
				outHtml2+='<tr><td>'+item.aaa401+'</td><td>'+item.aaa402+'</td><td>'+item.aaa404+'</td><td><a onclick="return confirm(\'请确认删除\');" href="/enterpriseportal/cps/brief?a=4&b=2&c='+item.aaa401+'" >删除</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="/enterpriseportal/cps/brief?b=1&c='+item.aaa401+'" >编辑</a></td></tr>';
			});//a=4团队管理b=1编辑 b=2删除
			$('.ue-clear').html('<div class="blue"><div id="slatenav"><ul><li><a href="/enterpriseportal/addarticlemodel/addteam?a=1" >新增文章</a></li></ul></div></div>'+outHtml1+outHtml2+outHtml3);
			$('.buttons').html('');
		});
	...
	if(mb!=null&&mc!=null){
		if(mb==2){//删除团队管理一项
			$.ajax({
				...
				success : function(data) {
						alert('修改成功');
						self.location='/enterpriseportal/cps/brief?a=4';
				}
			});
		}
		if(mb==1){//编辑团队管理一项
		...
		if(mb==3){
			$.getJSON(getHonoryByIdUrl+mc,function(data){
				...
				$('.buttons').html('<input class="button cabtn" type="button" value="确认修改" />&nbsp;&nbsp;&nbsp;&nbsp;<input class="button btn-return error" type="button" value="返回"/>');
				$('.btn-return').click(function(){
					self.location='/enterpriseportal/cps/brief?a=5';
				});
				$('.cabtn').click(function(){
	...
	if(ma!=null&&ma==5){//显示荣誉资质
		...
		$.getJSON(getHonorUrl,function(data){
			...
			honorObj.map(function(item,data){
				outHtml2+='<tr><td>'+item.aaa501+'</td><td>'+item.aaa502+'</td><td>'+item.aaa505+'</td><td><a onclick="return confirm(\'请确认删除\');" href="/enterpriseportal/cps/brief?a=5&b=4&c='+item.aaa501+'" >删除</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="/enterpriseportal/cps/brief?b=3&c='+item.aaa501+'" >编辑</a></td></tr>';
			});//a=5荣誉资质b=3编辑 b=4删除
			$('.ue-clear').html('<div class="blue"><div id="slatenav"><ul><li><a href="/enterpriseportal/addarticlemodel/addteam?a=2" >新增文章</a></li></ul></div></div>'+outHtml1+outHtml2+outHtml3);
			$('.buttons').html('');
		});	
	}
