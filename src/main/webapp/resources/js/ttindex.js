...
$(function() {
	//获得简介内容URL
	var getBriefIntroductionUrl='/enterpriseportal/frontend/getbriefintroduction';
	//获得主页tab四个图片URL
	var getTabUrl='/enterpriseportal/frontend/getindextabimg?tabid=';
	//获得主页tab四个标签内容标题、日期
	var getTabContent='/enterpriseportal/frontend/getindextabcontent?type=';
	var urlToresource='/';
	var getHonorUrl='/enterpriseportal/frontend/gethonor';
	var getBottomUrl='/enterpriseportal/frontend/getbottoma?bottomaid=';
	//荣誉资质页面标志
	...
	$.getJSON(getTabContent+1,function(data){
		...
		for(var i=0;i<data.projectsList.length;i++){
			...
			viewhtml=viewhtml+'<li><a href="/enterpriseportal/newstt/index?a=1&b='+
				typeid
	...
	$.getJSON(getTabContent+2,function(data){
		...
		for(var i=0;i<data.projectsList.length;i++){
			...
			if(ifview==0)
				continue;
			viewhtml=viewhtml+'<li><a href="/enterpriseportal/newstt/index?a=2&b='+
				typeid
				+'"><div class="am-u-sm-8 am-u-md-8 am-u-lg-8"><strong class="f-toe">'+
				title
				+'</strong></div><div class="am-u-sm-4 am-u-md-4 am-u-lg-4"><span>'+
	...
	$.getJSON(getTabContent+3,function(data){
		...
		for(var i=0;i<data.projectsList.length;i++){
			...
			if(ifview==0)
				continue;
			viewhtml=viewhtml+'<li><a href="/enterpriseportal/newstt/index?a=3&b='+
				typeid
				+'"><div class="am-u-sm-8 am-u-md-8 am-u-lg-8"><strong class="f-toe">'+
				title
				+'</strong></div><div class="am-u-sm-4 am-u-md-4 am-u-lg-4"><span>'+
	...
	$.getJSON(getTabContent+4,function(data){
		...
		for(var i=0;i<data.projectsList.length;i++){
			...
			if(ifview==0)
				continue;
			viewhtml=viewhtml+'<li><a href="/enterpriseportal/newstt/index?a=4&b='+
				typeid
				+'"><div class="am-u-sm-8 am-u-md-8 am-u-lg-8"><strong class="f-toe">'+
				title
				+'</strong></div><div class="am-u-sm-4 am-u-md-4 am-u-lg-4"><span>'+
	...
	$.getJSON(getHonorUrl,function(data){
		...
		honorList.map(function(item,index){
			if(item.aaa506!=0)
				viewHTML+='<li data-thumb-alt="" style="width: 200px; margin-right: 5px; float: left; display: block;"><a href="/enterpriseportal/companygx/index?a='+HONOR_PAGE_TAB+'&c='+item.aaa501+'"><img src="'+urlToresource+item.aaa503+'" /><span class="f-toe">'+item.aaa502+'</span></a></li>';
		});
	...
	$.getJSON(getBottomUrl+1,function(data){
		...
		bottoma.map(function(item,index){
			viewHTML+='<li><a href="/enterpriseportal/projectstt/index?a='+item.aac115+'&b='+item.aac101+'"><div class="am-u-sm-8 am-u-md-8 am-u-lg-8"><strong class="f-toe">'+
			item.aac102
			+'</strong></div><div class="am-u-sm-4 am-u-md-4 am-u-lg-4"><span>'+
			item.aac112
			+'</span></div></a></li>';
	...
	$.getJSON(getBottomUrl+2,function(data){
		...
		var viewHTML='';
		bottoma.map(function(item,index){
			viewHTML+='<li><a href="/enterpriseportal/projectstt/hr?a=2&b='+item.aac101
			+'"><div class="am-u-sm-8 am-u-md-8 am-u-lg-8"><strong class="f-toe">'+
			item.aac102
			+'</strong></div><div class="am-u-sm-4 am-u-md-4 am-u-lg-4"><span>'+
			item.aac112
